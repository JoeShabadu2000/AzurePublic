...
var nsgCustomRules = loadJsonContent('./nsgrules-unifi.json', 'securityRules')
...
resource vmCustomScriptResource 'Microsoft.Compute/virtualMachines/extensions@2021-11-01' = {
  ...
  properties: {
    ...
    protectedSettings: {
      commandToExecute: 'curl https://azurecliprod.blob.core.windows.net/install | bash && wget -O setup.sh ${vmSetupScriptURL} && bash setup.sh ${managedidentityClientID} ${vmTimeZone} ${vmKeyVaultName}'
    }
  }
}
